...
* Foundry-ML focuses foremost on accessibility and reproducibility. \autoref{fig1} shows an example of how, with just a few lines of code, researchers can access a curated collection of ML-ready datasets, the associated metadata describing the dataset contents, split details (e.g., train, test, validate), and other information (e.g., number of entries). Datasets are reviewed, shaped, and approved or denied in the curation process by the authors. As of Q1 2023, we have collected and made available 30 datasets in Foundry with data represented including tabular data (e.g., csv, Excel), key-value data (e.g, JSON), image sets, and hierarchical data (e.g., HDF5). 
...
* We have developed a prototype web interface (\autoref{fig2}) listing all datasets with instructions on how to access them and key features of each dataset (e.g., number of entries, inputs, targets, type of data, tags, free text description). 
